...
def execute_v4_trading_pipeline(live_execute=False):
    print("🎯 [라오어 무한매수법 V4.0 통합 주문 실행기 가동]")
    state = load_state()
    ...
    # 1. 매도(익절) 주문 생성 로직 (V4.0 최종 지정가 매도: SOXL +20%, TQQQ +15%)
    for code in ["SOXL", "TQQQ"]:
        if code in holdings:
            ...
            print(f"• 보유 종목 [{code}]: 수량 {qty}주 | 평단 ${avg_p:.2f} | 수익률 {pft:+.2f}% (목표: +{target_pct}%, 목표가: ${target_price:.2f})")

            if qty > 0 and pft >= target_pct:
                print(f"🎉 [{code}] V4.0 목표 수익률 +{target_pct}% 달성! 익절 지정가 매도 주문 생성")
                sell_input = {
                    "act_no": ACCOUNT_NO,
    ...
    # 2. 매수 (V4.0 별지점 + 큰수 매수 버퍼) 주문 생성 로직
    for code in ["SOXL", "TQQQ"]:
        t_val = state[code]["T"]
        ...
        # 큰수 매수 적용 (T=0 이거나 현재가와 별지점 차이가 클 때 매매거부 방지를 위해 버퍼 반영)
        buy_price = v4["buy_point"]
        if t_val == 0.0:
            # 새 사이클 시작 시: 큰수 매수 (현재가 + 10% ~ 15% 또는 별지점 중 높은 값/안전값)
            buy_price = round(cur_price * 1.12, 2) # 전일종가/현재가 +12% 큰수 매수 버퍼
        else:
            # 사이클 중간: 별지점 적용하되 매매거부 방지를 위해 현재가 대비 너무 낮으면 조정
            if buy_price < cur_price * 0.8:
                buy_price = round(cur_price * 0.95, 2)
        ...
        orders_to_place.append(("gbstockOrderBuy", buy_input))
        print(f"• [{code} V4.0 매수 가이드]: 회차 T={t_val:.1f} | 매수가(별지점/큰수): ${buy_price:.2f} | 수량: {shares}주")

    print(f"\n[실행할 V4.0 주문 리스트 ({len(orders_to_place)}건)]")
    for idx, (op_id, inp) in enumerate(orders_to_place, 1):
        print(f"--- 주문 #{idx} ({op_id}) ---")
        print(json.dumps(inp, indent=2, ensure_ascii=False))

        if live_execute:
            print(f"🚀 실계좌 V4.0 주문 전송 중...")
