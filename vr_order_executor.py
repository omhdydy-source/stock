...
def execute_vr_orders(live_execute=False):
    ...
    # 📌 스마트 락: 실계좌에 이미 2주짜리 예약 주문이 걸려있는지 체크
    has_orders = check_existing_reserved_orders()
    if has_orders:
        print("🛡️ [안전 가드] 계좌에 이미 활성화된 2주치 예약 주문 그물망이 존재하므로 중복 실행을 방지하기 위해 패스합니다.")
        return []
    ...
    # 엑셀(루나시트 스타일) 일지 및 차트 업데이트
    update_excel_log(rep)
    
    today_dt = datetime.now()
    end_dt = today_dt + timedelta(days=14) # 2주(14일) 동안 유효한 예약 주문
    today_str = today_dt.strftime("%Y%m%d")
    end_str = end_dt.strftime("%Y%m%d")
    
    orders_to_place = []
    
    # 🟢 매수 30단계 차등 예약 주문 생성 (2주 유효)
    ...
    print(f"\n[생성된 VR 30분할 매수/매도 2주 예약 주문 총 {len(orders_to_place)}건 (매수 30건 + 매도 30건)]")
